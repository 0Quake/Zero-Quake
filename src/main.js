...
function EEWcontrol(data) {
    ...
    if (!EEW_history[data.source].find(function(elm) {
    ...
    request.on("response", (res) => {
        ...
        res.on("end", function() {
            ...
            if (narikakun_URLs.length < 10 && first) {
                ...
                var index = 0;
                while (narikakun_EIDs.length < 10) {

                    var eidTmp = narikakun_URLs[index].split("_")[2]
    ...
    if (net.online) {
        while (!yoyuYOK) {
            await new Promise((resolve) => {
                ...
                request.on("response", (res) => {
                    //if (0 < loopCount) {
                    if (300 <= res._responseHead.statusCode || res._responseHead.statusCode < 200) {} else {
