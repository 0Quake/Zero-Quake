      ...
      try {
        ...
        Object.keys(json.station).forEach(function (StID) {
          ...
          if (stationData) {
            ...
            var rgb = KmoniColorTable[Math.max(-3, Math.floor(JPShindo * 10) / 10)];
            TremRtsData[StID] = {
              Type: "TREMRTS",
              shindo: JPShindo,
              PGA: st.pga,
...
function Req_kmoni() {//済
  ...
  var url = Kmoni_URLs[Kmoni_URLIndex].replace("[YYYYMMDD]", NormalizeDate(2, ReqTime)).replace("[YYYYMMDDhhmmss]", NormalizeDate(1, ReqTime));
  var request = net.request(url);
  request.on("response", (res) => {
    var dataTmp = [];
    res.on("data", (chunk) => {
    ...
    res.on("end", () => {
      try {
        if (300 <= res._responseHead.statusCode || res._responseHead.statusCode < 200) {
          ...
          Kmoni_ErrorCount = 0;
          if (WorkerWindow) {
            var imgBase64 = Buffer.concat(dataTmp).toString("base64");
            WorkerWindow.webContents.send("message2", {
...
function MargeSeisJS(json) {
  var rgb = KmoniColorTable[Math.max(-3, Math.floor(json.CalcShindo * 10) / 10)];
      ...
      if (!data.is_cancel) {
        ...
        if (data.depth <= 150) {
          ...
          JMA_Int_Points.forEach(function (elm) {
            if (elm.a && elm.s) {
              var sect = Object.keys(EQIAreaLoc)[elm.s];
              if (!sect) return;
              var estInt = calcInt(
                data.magnitude,
                data.depth,
